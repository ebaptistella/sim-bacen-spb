(ns com.github.ebaptistella.components.mq-worker
  ...
  (:require [com.github.ebaptistella.components.logger :as logger]
            ...
            [com.github.ebaptistella.controllers.message :as controllers.message]
            [com.github.ebaptistella.infrastructure.mq.consumer :as mq.consumer]
            [com.stuartsierra.component :as component]
            [schema.core :as s])
  (:import [java.util.concurrent Executors TimeUnit]))

(defn- run-worker-loop [run? poll-ms limit mq-cfg exec log]
  (while @run?
    (try
      ...
      (when @run?
        (let [messages (mq.consumer/receive-messages mq-cfg limit)]
          (when (seq messages)
            (logger/log-call log :info "[MQWorker] Processing %d message(s)" (count messages)))
          (doseq [msg messages]
            (.submit exec ^Runnable
                     (fn []
                       (try
                         (controllers.message/process! msg mq-cfg log)
                         (catch Exception e
                           (logger/log-call log :warn "[MQWorker] Failed to process message id=%s: %s"
...
(defrecord MQWorkerComponent [config logger poll-interval-ms thread-pool-size batch-limit executor thread running?]
  component/Lifecycle
  (start [this]
    (if thread
      this
      (let [log       (logger/bound logger)
            ...
            run?      (atom true)
            t         (doto (Thread. ^Runnable (fn [] (run-worker-loop run? poll-ms limit mq-cfg exec log)))
                        (.setName "mq-worker")
                        (.setDaemon true)
                        (.start))]
