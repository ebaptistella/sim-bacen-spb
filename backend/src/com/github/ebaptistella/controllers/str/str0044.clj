(ns com.github.ebaptistella.controllers.str.str0044
  "STR0044: Cancelamento de teste de contingência Internet. Fluxo1: R1 apenas."
  (:require [com.github.ebaptistella.components.logger :as logger]
            ...
            [com.github.ebaptistella.logic.str.str0044 :as logic.str0044])
  (:import [java.time Instant]))

(def ^:private accepted-response-types #{:STR0044R1})

(defmethod available-responses :STR0044 [_msg] [:STR0044R1])

(defmethod process! :STR0044
  [msg {:keys [store logger]}]
  (logger/log-call logger :info "[STR] STR0044 received | id=%s type=%s"
                   (:message-id msg) (:type msg))
  (store.messages/save! store msg))

(defmethod respond! :STR0044
  ...
  (cond
    ...
    :else
    (let [fields     (logic.str0044/r1-response msg (or params {}))
          xml        (logic.str0044/response->xml response-type fields)
