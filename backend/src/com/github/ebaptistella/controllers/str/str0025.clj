(ns com.github.ebaptistella.controllers.str.str0025
  "STR0025: TED para depósito judicial. Fluxo2: R1 + R2 + E."
  (:require [com.github.ebaptistella.components.logger :as logger]
            ...
            [com.github.ebaptistella.logic.str.str0025 :as logic.str0025])
  (:import [java.time Instant]))

(def ^:private accepted-response-types #{:STR0025R1 :STR0025R2 :STR0025E})

(defmethod available-responses :STR0025 [_msg] [:STR0025R1 :STR0025R2 :STR0025E])

(defmethod process! :STR0025
  [msg {:keys [store logger]}]
  (logger/log-call logger :info "[STR] STR0025 received | id=%s type=%s"
                   (:message-id msg) (:type msg))
  (store.messages/save! store msg))

(defmethod respond! :STR0025
  [msg {:keys [store mq-cfg]} {:keys [response-type params]}]
  (cond
    (and (= :responded (:status msg)) (not= :STR0025R2 response-type))
    {:error :already-responded}

    (not (accepted-response-types response-type))
    {:error :invalid-response-type}

    (and (= :responded (:status msg)) (= :STR0025R2 response-type) (store.messages/response-sent? msg :STR0025R2))
    {:error :r2-already-sent}

    (and (= :pending (:status msg)) (= :STR0025R2 response-type))
    {:error :r2-requires-r1}

    :else
    (let [p      (cond-> (or params {})
                   (= :STR0025R2 response-type)
                   (assoc :NumCtrlSTR (-> msg :responses first :num-ctrl-str)))
          fields (case response-type
                   :STR0025R1 (logic.str0025/r1-response msg p)
                   :STR0025R2 (logic.str0025/r2-response msg p)
                   :STR0025E  (logic.str0025/rejection-response msg p))]
      (if (= :missing-motivo (:error fields))
        fields
        (let [xml        (logic.str0025/response->xml response-type fields)
              queue-name (cond
                           (#{:STR0025R1 :STR0025E} response-type)
                           (parser/r1-outbound-queue (:queue-name msg))

                           (= :STR0025R2 response-type)
                           (parser/r2-outbound-queue (:queue-name msg) (:ispb-if-credtd msg)))]
          (when (nil? queue-name)
            (throw (ex-info "Cannot derive outbound queue for STR0025R2 (missing ISPBIFCredtd?)" {})))
          (mq.producer/send-message! mq-cfg queue-name xml)
          (if (= :STR0025R2 response-type)
