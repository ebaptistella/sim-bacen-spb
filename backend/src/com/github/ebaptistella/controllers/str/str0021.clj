(ns com.github.ebaptistella.controllers.str.str0021
  "STR0021: Repasse de FGTS. Fluxo2: R1 + R2 + E."
  (:require [com.github.ebaptistella.components.logger :as logger]
            ...
            [com.github.ebaptistella.logic.str.str0021 :as logic.str0021])
  (:import [java.time Instant]))

(def ^:private accepted-response-types #{:STR0021R1 :STR0021R2 :STR0021E})

(defmethod available-responses :STR0021 [_msg] [:STR0021R1 :STR0021R2 :STR0021E])

(defmethod process! :STR0021
  [msg {:keys [store logger]}]
  (logger/log-call logger :info "[STR] STR0021 received | id=%s type=%s"
                   (:message-id msg) (:type msg))
  (store.messages/save! store msg))

(defmethod respond! :STR0021
  [msg {:keys [store mq-cfg]} {:keys [response-type params]}]
  (cond
    (and (= :responded (:status msg)) (not= :STR0021R2 response-type))
    {:error :already-responded}

    (not (accepted-response-types response-type))
    {:error :invalid-response-type}

    (and (= :responded (:status msg)) (= :STR0021R2 response-type) (store.messages/response-sent? msg :STR0021R2))
    {:error :r2-already-sent}

    (and (= :pending (:status msg)) (= :STR0021R2 response-type))
    {:error :r2-requires-r1}

    :else
    (let [p      (cond-> (or params {})
                   (= :STR0021R2 response-type)
                   (assoc :NumCtrlSTR (-> msg :responses first :num-ctrl-str)))
          fields (case response-type
                   :STR0021R1 (logic.str0021/r1-response msg p)
                   :STR0021R2 (logic.str0021/r2-response msg p)
                   :STR0021E  (logic.str0021/rejection-response msg p))]
      (if (= :missing-motivo (:error fields))
        fields
        (let [xml        (logic.str0021/response->xml response-type fields)
              queue-name (cond
                           (#{:STR0021R1 :STR0021E} response-type)
                           (parser/r1-outbound-queue (:queue-name msg))

                           (= :STR0021R2 response-type)
                           (parser/r2-outbound-queue (:queue-name msg) (:ispb-if-credtd msg)))]
          (when (nil? queue-name)
            (throw (ex-info "Cannot derive outbound queue for STR0021R2 (missing ISPBIFCredtd?)" {})))
          (mq.producer/send-message! mq-cfg queue-name xml)
          (if (= :STR0021R2 response-type)
