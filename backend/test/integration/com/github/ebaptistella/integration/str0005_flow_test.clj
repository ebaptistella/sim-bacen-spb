(ns com.github.ebaptistella.integration.str0005-flow-test
...
(defn- sample-str0005-msg
  ...
  {:id              id
   ...
   :message-id      "mq-test-0005"
   :body            "<STR0005><CodMsg>STR0005</CodMsg></STR0005>"
...
(defflow str0005-respond-r1
  {:init flow-init}
  (flow "POST respond STR0005R1 — 200, store :responded, MQ QR.* receives R1 XML"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     capture  (sf/invoke #(atom nil))
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0005-msg id) :id id)))
     resp     (sf/invoke #(with-redefs [mq.producer/send-message!
                            ...
                            (http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                            {:response-type "STR0005R1"})))]
    (match? 200 (sf/invoke #(:status resp)))
    (match? :responded (sf/invoke #(-> (store.messages/find-by-id store id) :status)))
    (match? :STR0005R1 (sf/invoke #(-> (store.messages/find-by-id store id) :responses first :type)))
    ...
    (match? true (sf/invoke #(str/includes? (:xml @capture) "STR0005R1")))
    (match? true (sf/invoke #(str/includes? (:xml @capture) "LQDADO")))))

(defflow str0005-respond-404
  {:init flow-init}
  ...
  (flow "POST respond already-responded message — 409, store unchanged"
    [store    (sf/get-state :store)
     ...
     _        (sf/invoke #(store.messages/save! store
                                                 (-> (sample-str0005-msg id)
                                                     (assoc :status :responded
                                                            :responses [{:type    :STR0005R1
                                                                         :body    "<x/>"
                                                                         :sent-at "t"}]))))
...
(defflow str0005-respond-e-sem-motivo
  {:init flow-init}
  (flow "POST STR0005E without MotivoRejeicao — 400, status remains :pending"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0005-msg id) :id id)))
     resp     (sf/invoke #(http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                          {:response-type "STR0005E"}))]
...
(defflow str0005-r2-before-r1
  {:init flow-init}
  (flow "POST STR0005R2 on pending message — 422, status remains :pending"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0005-msg id) :id id)))
     resp     (sf/invoke #(http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                          {:response-type "STR0005R2"}))]
    (match? 422 (sf/invoke #(:status resp)))
    (match? :pending (sf/invoke #(-> (store.messages/find-by-id store id) :status)))
    (match? nil (sf/invoke #(-> (store.messages/find-by-id store id) :responses)))))

(defflow str0005-r1-then-r2-same-num-ctrl-str
  {:init flow-init}
  (flow "POST R1 then R2 — R2 XML contains same NumCtrlSTR as R1, R2 goes to creditor queue"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
