(ns com.github.ebaptistella.integration.str0034-flow-test
...
(defn- sample-str0034-msg
  ...
  {:id             id
   ...
   :message-id     "mq-test-0034"
   :body           "<STR0034><CodMsg>STR0034</CodMsg></STR0034>"
...
(defflow str0034-respond-r1
  {:init flow-init}
  (flow "POST respond STR0034R1 — 200, store :responded, XML contains STR0034R1 and LQDADO"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     capture  (sf/invoke #(atom nil))
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0034-msg id) :id id)))
     resp     (sf/invoke #(with-redefs [mq.producer/send-message!
                            ...
                            (http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                            {:response-type "STR0034R1"})))]
    (match? 200 (sf/invoke #(:status resp)))
    (match? :responded (sf/invoke #(-> (store.messages/find-by-id store id) :status)))
    (match? :STR0034R1 (sf/invoke #(-> (store.messages/find-by-id store id) :responses first :type)))
    (match? "QR.REQ.99999999.00000000.01" (sf/invoke #(:queue @capture)))
    (match? true (sf/invoke #(str/includes? (:xml @capture) "LQDADO")))))

(defflow str0034-respond-r1-then-r2-finldd-if
  {:init flow-init}
  (flow "POST R1 then R2 — R2 XML has FinlddIF, correct queue, same NumCtrlSTR"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     ...
     _        (sf/invoke #(with-redefs [mq.producer/send-message!
                            ...
                            (http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                            {:response-type "STR0034R2"})))]
    (match? true (sf/invoke #(str/includes? (:r2-xml @capture) "<FinlddIF>00006</FinlddIF>")))
...
(defflow str0034-respond-e-com-motivo
  {:init flow-init}
  (flow "POST STR0034E with MotivoRejeicao — 200, store :responded"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     capture  (sf/invoke #(atom nil))
     id       (sf/invoke #(str (random-uuid)))
     ...
     resp     (sf/invoke #(with-redefs [mq.producer/send-message!
                                        (fn [_ q body] (reset! capture {:queue q :xml body}))]
                            (http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                            {:response-type "STR0034E"
                                             :params        {"MotivoRejeicao" "AC09"}})))]
    (match? 200 (sf/invoke #(:status resp)))
    (match? :responded (sf/invoke #(-> (store.messages/find-by-id store id) :status)))
    (match? true (sf/invoke #(str/includes? (:xml @capture) "AC09")))))

(defflow str0034-respond-e-sem-motivo
  {:init flow-init}
  (flow "POST STR0034E without MotivoRejeicao — 400, status remains :pending"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0034-msg id) :id id)))
...
(defflow str0034-respond-409
  {:init flow-init}
  (flow "POST respond already-responded — 409"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store
                                                (-> (sample-str0034-msg id)
                                                    (assoc :status :responded
                                                           :responses [{:type :STR0034R1 :body "<x/>" :sent-at "t"}]))))
     resp     (sf/invoke #(http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                          {:response-type "STR0034R1"}))]
    (match? 409 (sf/invoke #(:status resp)))))

(defflow str0034-r2-before-r1
  {:init flow-init}
  (flow "POST STR0034R2 on pending message — 422"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0034-msg id) :id id)))
