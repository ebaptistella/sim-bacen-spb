(ns com.github.ebaptistella.integration.str0003-flow-test
  (:require [cheshire.core :as json]
            [clojure.string :as str]
            [clojure.test :refer [use-fixtures]]
            com.github.ebaptistella.controllers.str.str0003
...
(defn- sample-str0003-msg
  ...
  {:id             id
   ...
   :message-id     "mq-test-0003"
   :body           "<STR0003><CodMsg>STR0003</CodMsg></STR0003>"
...
(defflow str0003-respond-r1
  {:init flow-init}
  (flow "POST respond STR0003R1 — 200, store :responded, queue QR.*"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     capture  (sf/invoke #(atom nil))
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0003-msg id) :id id)))
...
(defflow str0003-respond-r1-then-r2-finldd-if
  {:init flow-init}
  (flow "POST R1 then R2 — R2 XML has FinlddIF, correct queue"
    [store    (sf/get-state :store)
     ...
     _        (sf/invoke #(with-redefs [mq.producer/send-message!
                            ...
                            (http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                            {:response-type "STR0003R2"})))]
    (match? true (sf/invoke #(str/includes? (:r2-xml @capture) "<FinlddIF>00003</FinlddIF>")))
...
(defflow str0003-respond-e-com-motivo
  {:init flow-init}
  (flow "POST STR0003E with MotivoRejeicao — 200, store :responded"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0003-msg id) :id id)))
     resp     (sf/invoke #(with-redefs [mq.producer/send-message! (fn [_ _ _] nil)]
                            (http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                            {:response-type "STR0003E"
                                             :params        {"MotivoRejeicao" "AC09"}})))]
    (match? 200 (sf/invoke #(:status resp)))
    (match? :responded (sf/invoke #(-> (store.messages/find-by-id store id) :status)))))

(defflow str0003-respond-e-sem-motivo
  {:init flow-init}
  (flow "POST STR0003E without MotivoRejeicao — 400, store :pending"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0003-msg id) :id id)))
...
(defflow str0003-r2-before-r1
  {:init flow-init}
  (flow "POST STR0003R2 on pending — 422"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0003-msg id) :id id)))
