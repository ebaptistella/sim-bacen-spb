(ns com.github.ebaptistella.integration.str0010-flow-test
...
(defn- sample-str0010-msg
  ...
  {:id              id
   ...
   :message-id      "mq-test-0010"
   :body            "<STR0010><CodMsg>STR0010</CodMsg></STR0010>"
...
(defflow str0010-respond-r1
  {:init flow-init}
  (flow "POST respond STR0010R1 — 200, store :responded, MQ QR.* receives LQDADO XML"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     capture  (sf/invoke #(atom nil))
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0010-msg id) :id id)))
     resp     (sf/invoke #(with-redefs [mq.producer/send-message!
                            ...
                            (http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                            {:response-type "STR0010R1"})))]
    (match? 200 (sf/invoke #(:status resp)))
    (match? :responded (sf/invoke #(-> (store.messages/find-by-id store id) :status)))
    (match? :STR0010R1 (sf/invoke #(-> (store.messages/find-by-id store id) :responses first :type)))
    ...
    (match? true (sf/invoke #(str/includes? (:xml @capture) "STR0010R1")))
    (match? true (sf/invoke #(str/includes? (:xml @capture) "LQDADO")))))

(defflow str0010-r1-then-r2-same-num-ctrl-str
  {:init flow-init}
...
(defflow str0010-r2-includes-cod-dev-transf
  {:init flow-init}
  (flow "POST STR0010R2 — XML includes CodDevTransf from original message"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     capture  (sf/invoke #(atom nil))
     id       (sf/invoke #(str (random-uuid)))
     ...
     _        (sf/invoke #(with-redefs [mq.producer/send-message!
                            ...
                            (http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                            {:response-type "STR0010R2"})))]
    (match? true (sf/invoke #(str/includes? @capture "<CodDevTransf>MD06</CodDevTransf>")))))

(defflow str0010-respond-e-com-motivo
  {:init flow-init}
  (flow "POST STR0010E with MotivoRejeicao — 200, store :responded, XML contains motivo"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     capture  (sf/invoke #(atom nil))
     id       (sf/invoke #(str (random-uuid)))
...
(defflow str0010-respond-e-sem-motivo
  {:init flow-init}
  (flow "POST STR0010E without MotivoRejeicao — 400, status remains :pending"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0010-msg id) :id id)))
...
(defflow str0010-r2-before-r1
  {:init flow-init}
  (flow "POST STR0010R2 before R1 — 422, status remains :pending"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0010-msg id) :id id)))
...
(defflow str0010-respond-409
  {:init flow-init}
  (flow "POST respond already-responded STR0010 — 409, store unchanged"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store
                                                 (-> (sample-str0010-msg id)
                                                     (assoc :status :responded
                                                            :responses [{:type    :STR0010R1
                                                                         :body    "<x/>"
                                                                         :sent-at "t"}]))))
...
(defflow str0010-mq-failure
  {:init flow-init}
  (flow "MQ send failure on STR0010R1 — 500, store remains :pending"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0010-msg id) :id id)))
...
(defflow str0010-r2-already-sent
  {:init flow-init}
  (flow "POST STR0010R2 twice — 409 on second attempt"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0010-msg id) :id id)))
...
(defflow str0010-get-available-responses-pending
  {:init flow-init}
  (flow "GET /api/v1/messages/:id — STR0010 pending returns R1, R2, E"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0010-msg id) :id id)))
    ...
    (match? 200 (sf/invoke #(:status resp)))
    (match? ["STR0010R1" "STR0010R2" "STR0010E"]
            (sf/invoke #(-> resp :body :data :available-responses)))))

(defflow str0010-get-available-responses-after-r1
  {:init flow-init}
  (flow "GET /api/v1/messages/:id — STR0010 after R1 returns only R2"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-str0010-msg id) :id id)))
