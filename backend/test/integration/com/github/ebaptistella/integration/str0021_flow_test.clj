(ns com.github.ebaptistella.integration.str0021-flow-test
  (:require [cheshire.core :as json]
            [clojure.string :as str]
            [clojure.test :refer [use-fixtures]]
            com.github.ebaptistella.controllers.str.str0021
...
(defn- sample-msg [id]
  {:id             id
   ...
   :message-id     "mq-test-0021"
   :body           "<STR0021><CodMsg>STR0021</CodMsg></STR0021>"
...
(defflow str0021-r1-smoke
  {:init flow-init}
  (flow "POST STR0021R1 — 200, LQDADO in XML"
  ...
  (flow "POST R1 then R2 — FinlddIF in R2 XML"
    [store    (sf/get-state :store)
     ...
     _        (sf/invoke #(with-redefs [mq.producer/send-message!
                            ...
                            (http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                            {:response-type "STR0021R2"})))]
    (match? true (sf/invoke #(str/includes? @capture "<FinlddIF>00021</FinlddIF>")))))

(defflow str0021-e-sem-motivo-smoke
  {:init flow-init}
  (flow "POST STR0021E without MotivoRejeicao — 400"
    [store    (sf/get-state :store)
     base-url (sf/get-state :base-url)
     id       (sf/invoke #(str (random-uuid)))
     _        (sf/invoke #(store.messages/save! store (assoc (sample-msg id) :id id)))
     resp     (sf/invoke #(http-post-json base-url (str "/api/v1/messages/" id "/respond")
                                          {:response-type "STR0021E"}))]
